...
def main_screen():
    ...
    def show():
        if screen.call("ttk::style", "theme", "use") == "x-dark":
            ...
            if pswrd.cget('show') == '*':
                pswrd.config(show = "")
                eye_B.configure(image = hide_img)

            else:
                pswrd.config(show = "*")
                eye_B.configure(image = show_img)
        else:
    ...
    dark_img = Image.open("./resource/dark-m.png")
    light_img = Image.open("./resource/light-m.png")
    my_image = CTkImage(light_image=light_img,
                        dark_image=dark_img)


    theme_B = CTkButton(screen,
                image = my_image,text = "",hover = FALSE,height = 20,width = 20,
                fg_color = ("#fafafa","#1c1c1c"),bg_color = ("#fafafa","#1c1c1c"),
                command = theme_change)
    theme_B.place(x = 12, y = 95)

    black_show_img = Image.open("./resource/black_show.png")
    black_hide_img = Image.open("./resource/black_hide.png")
    white_show_img = Image.open("./resource/white_show.png")
    white_hide_img = Image.open("./resource/white_hide.png")
    
    show_img = CTkImage(light_image = black_show_img,
                        dark_image = white_show_img)
    hide_img = CTkImage(light_image = black_hide_img,
                        dark_image = white_hide_img)


    eye_B = CTkButton(screen,
                image = show_img,text = "",hover = FALSE,height = 10,width = 10,
                fg_color = ("#fafafa","#1c1c1c"),bg_color = ("#fafafa","#1c1c1c"),
                command = show)
    eye_B.place(x = 314,y = 50)
